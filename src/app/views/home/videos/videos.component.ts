...
export class VideosComponent implements OnInit {
  ...
  private categoryById = new Map<number, CategoryModel>();
  ...
  getCategoryLabel(categoryId: number): string {
    return this.categoryById.get(categoryId)?.title ?? `ID ${categoryId}`;
  }

  getCategoryBadgeStyle(categoryId: number): Record<string, string> {
  ...
  private applyFilters(): void {
    ...
    this.filteredVideos = this.videos.filter((video) => {
      const matchesCategory = this.selectedCategoryId === null || video.categoryId === this.selectedCategoryId;
      ...
      const category = this.getCategoryLabel(video.categoryId).toLowerCase();
