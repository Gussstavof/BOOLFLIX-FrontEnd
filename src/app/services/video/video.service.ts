...
export class VideoService {
  ...
  createVideo(video: Omit<VideoModel, 'id' | 'urlSafe'>): Observable<VideoModel> {
    return this.httpClient.post<VideoModel>(this.URL, video, this.httpOptions);
  }

  updateVideo(id: string, video: Partial<Omit<VideoModel, 'id' | 'urlSafe'>>): Observable<VideoModel> {
  ...
  searchVideos(search: string): Observable<PageableModel<VideoModel>> {
    return this.httpClient.get<PageableModel<VideoModel>>(`${this.URL}?search=${encodeURIComponent(search)}`, this.httpOptions);
  }
}
